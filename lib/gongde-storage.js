...
export function getGongdeLevel(total, lang = "zh") {
  const value = Number(total) || 0;
  const en = lang === "en";

  if (value < 10) {
    return { key: "starter", label: en ? "Fresh Start" : "清心入门", range: "0-9" };
  }

  if (value < 100) {
    return { key: "steady", label: en ? "Steady Mind" : "心态稳定", range: "10-99" };
  }

  if (value < 500) {
    return { key: "focused", label: en ? "Rising Merit" : "功德进阶", range: "100-499" };
  }

  return { key: "master", label: en ? "Full Merit" : "功德圆满", range: "500+" };
}
