...
不要使用 generator 或 executor 的聊天历史。以 intake 产物、引用的源码指针和明确的用户决策为主要事实来源；必要时可以使用工具、相关能力、代码/文档阅读、subagent 或 MCP 来核实澄清质量。

不要把 runner 历史、workflow 内部资料、无关本地 skill 文档或历史会话当作当前项目事实来源。隔离聊天历史不等于禁用工具、代码阅读、文档阅读、skill、subagent 或 MCP 能力；关键是所有判断都要能回到当前项目事实、用户决策或明确外部来源。
